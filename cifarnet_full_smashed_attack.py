"""
Full-smashed-representation inversion attack for the original UnSplit CifarNet.

This script follows the same attack structure as mobilenetv1_full_smashed_attack.py,
but uses the CifarNet model from models.py. The victim is trained as a split
client/server pair for the requested UnSplit split layer, matching the original
CifarNet split-execution style.
"""
...
def full_smashed_inversion_attack(
    ...
    for main_iter in range(main_iters):
        ...
        clone_client.eval()
        with torch.no_grad():
            match_loss = mse(clone_client(x_hat, end=split_layer), target_s).item()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CifarNet full-smashed UnSplit-style attack.")
    parser.add_argument("--data-root", type=str, default="data/cifar")
    parser.add_argument("--split-layer", type=int, required=True,
                        help="Original UnSplit CifarNet split layer index. Use 1-6 for Table-2-style runs.")
    ...
    parser.add_argument("--input-change-tol", type=float, default=1e-7)
    ...
    if not 1 <= args.split_layer <= 6:
        raise SystemExit("ERROR: Use --split-layer 1 through 6 for Table-2-style CifarNet runs.")
    ...
    result, attack_iters = full_smashed_inversion_attack(
    ...
    save_dir = Path(args.save_dir) if args.save_dir else Path(f"results_cifarnet_{mode}_split{args.split_layer}")
